...
export class ReplaceRobotcallPhoneRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  productInstanceId?: string;
  // 请求ID
...
export class ReplaceRobotcallPhoneResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 外呼手机号解密
   */
  async replaceRobotcallPhone(request: ReplaceRobotcallPhoneRequest): Promise<ReplaceRobotcallPhoneResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.replaceRobotcallPhoneEx(request, headers, runtime);
  }
   ...
   * Summary: 外呼手机号解密
   */
  async replaceRobotcallPhoneEx(request: ReplaceRobotcallPhoneRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<ReplaceRobotcallPhoneResponse> {
    Util.validateModel(request);
    return $tea.cast<ReplaceRobotcallPhoneResponse>(await this.doRequest("1.0", "antdigital.unicontact.robotcall.phone.replace", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new ReplaceRobotcallPhoneResponse({}));
  }
