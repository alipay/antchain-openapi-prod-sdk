...
export class CallbackMktMonitordataRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  // 回传请求对应id
  requestId: string;
...
export class CallbackMktMonitordataResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 保司营销监测数据回传
   */
  async callbackMktMonitordata(request: CallbackMktMonitordataRequest): Promise<CallbackMktMonitordataResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.callbackMktMonitordataEx(request, headers, runtime);
  }
