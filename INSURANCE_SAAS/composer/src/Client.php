...
use AntChain\INSURANCE_SAAS\Models\CallbackMktMonitordataRequest;
use AntChain\INSURANCE_SAAS\Models\CallbackMktMonitordataResponse;
use AntChain\INSURANCE_SAAS\Models\NotifyEmbedoemautoinsuranceEventRequest;
use AntChain\INSURANCE_SAAS\Models\NotifyEmbedoemautoinsuranceEventResponse;
use AntChain\INSURANCE_SAAS\Models\CallbackMktMediastatisticaldataRequest;
use AntChain\INSURANCE_SAAS\Models\CallbackMktMediastatisticaldataResponse;
use AntChain\INSURANCE_SAAS\Models\CallbackMktEffectsimpleRequest;
...
class Client {
     ...
     * Summary: 保司营销监测数据回传
     * @param CallbackMktMonitordataRequest $request
     * @return CallbackMktMonitordataResponse
     */
    public function callbackMktMonitordata($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->callbackMktMonitordataEx($request, $headers, $runtime);
    }
     ...
     * @return CallbackMktMonitordataResponse
     */
    public function callbackMktMonitordataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return CallbackMktMonitordataResponse::fromMap($this->doRequest("1.0", "antcloud.insurance.mkt.monitordata.callback", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 理想蚂蚁保项目
     * Summary: 理想蚂蚁保项目
     * @param NotifyEmbedoemautoinsuranceEventRequest $request
     * @return NotifyEmbedoemautoinsuranceEventResponse
     */
    public function notifyEmbedoemautoinsuranceEvent($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->notifyEmbedoemautoinsuranceEventEx($request, $headers, $runtime);
    }
     ...
     * @return NotifyEmbedoemautoinsuranceEventResponse
     */
    public function notifyEmbedoemautoinsuranceEventEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return NotifyEmbedoemautoinsuranceEventResponse::fromMap($this->doRequest("1.0", "antcloud.insurance.embedoemautoinsurance.event.notify", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
     ...
     */
    public function callbackMktMediastatisticaldata($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->callbackMktMediastatisticaldataEx($request, $headers, $runtime);
    }
     ...
     * @return CallbackMktMediastatisticaldataResponse
     */
    public function callbackMktMediastatisticaldataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return CallbackMktMediastatisticaldataResponse::fromMap($this->doRequest("1.0", "antcloud.insurance.mkt.mediastatisticaldata.callback", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 营销效果节点数据（简版）
     * Summary: 营销效果节点数据（简版）
     * @param CallbackMktEffectsimpleRequest $request
     * @return CallbackMktEffectsimpleResponse
     */
    public function callbackMktEffectsimple($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->callbackMktEffectsimpleEx($request, $headers, $runtime);
    }
     ...
     * @return CallbackMktEffectsimpleResponse
     */
    public function callbackMktEffectsimpleEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return CallbackMktEffectsimpleResponse::fromMap($this->doRequest("1.0", "antcloud.insurance.mkt.effectsimple.callback", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 保险营销服务商人群圈选
     * Summary: 保险营销服务商人群圈选
     * @param ApplyMktserviceproviderAudiencecircleRequest $request
     * @return ApplyMktserviceproviderAudiencecircleResponse
     */
    public function applyMktserviceproviderAudiencecircle($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
    ...
    public function uploadMktFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadMktFileResponse = new UploadMktFileResponse([
