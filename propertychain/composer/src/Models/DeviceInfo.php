...
class DeviceInfo extends Model
{
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['device_group_list'])) {
            if (!empty($map['device_group_list'])) {
                $model->deviceGroupList = [];
                $n                      = 0;
                foreach ($map['device_group_list'] as $item) {
                    $model->deviceGroupList[$n++] = null !== $item ? DeviceGroup::fromMap($item) : $item;
                }
            }
        }
