...
export class QueryBlockchainAppexAsynformStatusResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 单据上链状态查询
   */
  async queryBlockchainAppexAsynformStatus(request: QueryBlockchainAppexAsynformStatusRequest): Promise<QueryBlockchainAppexAsynformStatusResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryBlockchainAppexAsynformStatusEx(request, headers, runtime);
  }
