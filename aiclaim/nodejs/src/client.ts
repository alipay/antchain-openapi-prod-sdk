...
export class ExecImageClassificationRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
...
export class ExecImageClassificationResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 理赔材料照片分类
   */
  async execImageClassification(request: ExecImageClassificationRequest): Promise<ExecImageClassificationResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.execImageClassificationEx(request, headers, runtime);
  }
   ...
   * Summary: 理赔材料照片分类
   */
  async execImageClassificationEx(request: ExecImageClassificationRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<ExecImageClassificationResponse> {
    if (!Util.isUnset(request.fileObject)) {
      ...
      if (!AntchainUtil.isSuccess(uploadResp.resultCode, "ok")) {
        let execImageClassificationResponse = new ExecImageClassificationResponse({
