// This file is auto-generated, don't edit it. Thanks.
package com.antgroup.antchain.openapi.riskplus.models;

import com.aliyun.tea.*;

public class NotifyDubbridgeRepaymentplanLxResponse extends TeaModel {
    // 请求唯一ID，用于链路跟踪和问题排查
    @NameInMap("req_msg_id")
    public String reqMsgId;

    // 结果码，一般OK表示调用成功
    @NameInMap("result_code")
    public String resultCode;

    // 异常信息的文本描述
    @NameInMap("result_msg")
    public String resultMsg;

    // 同步结果，0-成功 1-失败
    @NameInMap("status")
    public Long status;

    // 错误描述
    @NameInMap("msg")
    public String msg;

    public static NotifyDubbridgeRepaymentplanLxResponse build(java.util.Map<String, ?> map) throws Exception {
        NotifyDubbridgeRepaymentplanLxResponse self = new NotifyDubbridgeRepaymentplanLxResponse();
        return TeaModel.build(map, self);
    }

    public NotifyDubbridgeRepaymentplanLxResponse setReqMsgId(String reqMsgId) {
        this.reqMsgId = reqMsgId;
        return this;
    }
    public String getReqMsgId() {
        return this.reqMsgId;
    }

    public NotifyDubbridgeRepaymentplanLxResponse setResultCode(String resultCode) {
        this.resultCode = resultCode;
        return this;
    }
    public String getResultCode() {
        return this.resultCode;
    }

    public NotifyDubbridgeRepaymentplanLxResponse setResultMsg(String resultMsg) {
        this.resultMsg = resultMsg;
        return this;
    }
    public String getResultMsg() {
        return this.resultMsg;
    }

    public NotifyDubbridgeRepaymentplanLxResponse setStatus(Long status) {
        this.status = status;
        return this;
    }
    public Long getStatus() {
        return this.status;
    }

    public NotifyDubbridgeRepaymentplanLxResponse setMsg(String msg) {
        this.msg = msg;
        return this;
    }
    public String getMsg() {
        return this.msg;
    }

}
