...
export class AWorkbenchTestRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  // 超时时间
  timeout: string;
...
export class AWorkbenchTestResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 工作台测试产品A
   */
  async aWorkbenchTest(request: AWorkbenchTestRequest): Promise<AWorkbenchTestResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.aWorkbenchTestEx(request, headers, runtime);
  }
   ...
   * Summary: 工作台测试产品A
   */
  async aWorkbenchTestEx(request: AWorkbenchTestRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<AWorkbenchTestResponse> {
    Util.validateModel(request);
    return $tea.cast<AWorkbenchTestResponse>(await this.doRequest("1.0", "antcloud.cat.workbench.test.a", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new AWorkbenchTestResponse({}));
  }
