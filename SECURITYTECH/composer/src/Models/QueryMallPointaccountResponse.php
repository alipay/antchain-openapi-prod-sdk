...
use AntChain\SECURITYTECH\Models\PointAccountInfo;

class QueryMallPointaccountResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['account_info_list'])){
            if(!empty($map['account_info_list'])){
                ...
                foreach($map['account_info_list'] as $item) {
                    $model->accountInfoList[$n++] = null !== $item ? PointAccountInfo::fromMap($item) : $item;
                }
            }
        }
        return $model;
