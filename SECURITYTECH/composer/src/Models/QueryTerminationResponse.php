...
use AntChain\SECURITYTECH\Models\AccountInfo;

class QueryTerminationResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['account_info_list'])){
            if(!empty($map['account_info_list'])){
                ...
                foreach($map['account_info_list'] as $item) {
                    $model->accountInfoList[$n++] = null !== $item ? AccountInfo::fromMap($item) : $item;
                }
            }
        }
        return $model;
