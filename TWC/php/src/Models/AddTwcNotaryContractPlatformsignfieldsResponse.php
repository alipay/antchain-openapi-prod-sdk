...
use AntChain\TWC\Models\ContractSignField;

class AddTwcNotaryContractPlatformsignfieldsResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['signfields'])){
            if(!empty($map['signfields'])){
                ...
                foreach($map['signfields'] as $item) {
                    $model->signfields[$n++] = null !== $item ? ContractSignField::fromMap($item) : $item;
                }
            }
        }
        return $model;
