...
use AntChain\TWC\Models\ContractSignFieldDetail;

class QueryContractSignfieldsResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['signfields'])){
            if(!empty($map['signfields'])){
                ...
                foreach($map['signfields'] as $item) {
                    $model->signfields[$n++] = null !== $item ? ContractSignFieldDetail::fromMap($item) : $item;
                }
            }
        }
        return $model;
