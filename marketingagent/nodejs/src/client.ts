...
export class GetComsumeJdRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  // 项目ID
  projectId: string;
...
export class GetComsumeJdResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 京东微信核销数据回调
   */
  async getComsumeJd(request: GetComsumeJdRequest): Promise<GetComsumeJdResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.getComsumeJdEx(request, headers, runtime);
  }
   ...
   * Summary: 京东微信核销数据回调
   */
  async getComsumeJdEx(request: GetComsumeJdRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<GetComsumeJdResponse> {
    Util.validateModel(request);
    return $tea.cast<GetComsumeJdResponse>(await this.doRequest("1.0", "antcloud.marketingagent.comsume.jd.get", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new GetComsumeJdResponse({}));
  }
