...
export class QueryTestCreateRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  static names(): { [key: string]: string } {
    return {
...
export class QueryTestCreateResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   */
  async queryTestCreate(request: QueryTestCreateRequest): Promise<QueryTestCreateResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryTestCreateEx(request, headers, runtime);
  }
   ...
   * Summary: 用于个人工作台二期测试使用
   */
  async queryTestCreateEx(request: QueryTestCreateRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<QueryTestCreateResponse> {
    Util.validateModel(request);
    return $tea.cast<QueryTestCreateResponse>(await this.doRequest("1.0", "antcloud.tritestdemo.test.create.query", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new QueryTestCreateResponse({}));
  }
