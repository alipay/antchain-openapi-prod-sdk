...
export class BindAntcloudCatHuanyuRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  // huanyu
  operator: string;
...
export class BindAntcloudCatHuanyuResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: huanyu
   */
  async bindAntcloudCatHuanyu(request: BindAntcloudCatHuanyuRequest): Promise<BindAntcloudCatHuanyuResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.bindAntcloudCatHuanyuEx(request, headers, runtime);
  }
   ...
   * Summary: huanyu
   */
  async bindAntcloudCatHuanyuEx(request: BindAntcloudCatHuanyuRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<BindAntcloudCatHuanyuResponse> {
    Util.validateModel(request);
    return $tea.cast<BindAntcloudCatHuanyuResponse>(await this.doRequest("1.0", "antcloud.cat.huanyu.bind", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new BindAntcloudCatHuanyuResponse({}));
  }
