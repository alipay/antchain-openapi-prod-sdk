...
use AntChain\IOTAGENT\Models\ProductKeyModel;

class ProductKeyPageResponse extends Model {
    protected $_name = [
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? ProductKeyModel::fromMap($item) : $item;
                }
            }
        }
        return $model;
