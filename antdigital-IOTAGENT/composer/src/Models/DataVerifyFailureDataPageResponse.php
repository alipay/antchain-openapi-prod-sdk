...
use AntChain\IOTAGENT\Models\DataVerifyFailureData;

class DataVerifyFailureDataPageResponse extends Model {
    protected $_name = [
    ...
    /**
     * @param array $map
     * @return DataVerifyFailureDataPageResponse
     */
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? DataVerifyFailureData::fromMap($item) : $item;
                }
            }
        }
        return $model;
