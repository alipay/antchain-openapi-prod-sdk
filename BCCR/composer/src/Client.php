...
use AntChain\BCCR\Models\VerifyDciCertRequest;
use AntChain\BCCR\Models\VerifyDciCertResponse;
use AntChain\BCCR\Models\ResultDciCertRequest;
use AntChain\BCCR\Models\ResultDciCertResponse;
use AntChain\BCCR\Models\QueryapplyformCopyrightregistrationDigitalregistrationRequest;
use AntChain\BCCR\Models\QueryapplyformCopyrightregistrationDigitalregistrationResponse;
use AntChain\BCCR\Models\QueryautosealauthCopyrightregistrationDigitalregistrationRequest;
use AntChain\BCCR\Models\QueryautosealauthCopyrightregistrationDigitalregistrationResponse;
use AntChain\BCCR\Models\CreateapplyformsealflowCopyrightregistrationDigitalregistrationRequest;
use AntChain\BCCR\Models\CreateapplyformsealflowCopyrightregistrationDigitalregistrationResponse;
use AntChain\BCCR\Models\QueryapplyformsealurlCopyrightregistrationDigitalregistrationRequest;
...
use AntChain\BCCR\Models\VerifyDciCertificateRequest;
...
class Client {
     ...
     * @param VerifyDciCertRequest $request
     * @return VerifyDciCertResponse
     */
    public function verifyDciCert($request){
     ...
     * @return VerifyDciCertResponse
     */
    public function verifyDciCertEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return VerifyDciCertResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.dci.cert.verify", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 查询DCI凭证核验结果
     * Summary: 查询DCI凭证核验结果
     * @param ResultDciCertRequest $request
     * @return ResultDciCertResponse
     */
    public function resultDciCert($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return ResultDciCertResponse
     */
    public function resultDciCertEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return ResultDciCertResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.dci.cert.result", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 版权登记-数登申请-查询数登申请表
     * Summary: 版权登记-数登申请-查询数登申请表
     * @param QueryapplyformCopyrightregistrationDigitalregistrationRequest $request
     * @return QueryapplyformCopyrightregistrationDigitalregistrationResponse
     */
    public function queryapplyformCopyrightregistrationDigitalregistration($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryapplyformCopyrightregistrationDigitalregistrationResponse
     */
    public function queryapplyformCopyrightregistrationDigitalregistrationEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryapplyformCopyrightregistrationDigitalregistrationResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.copyrightregistration.digitalregistration.queryapplyform", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 查询当前数登申请是否支持静默签授权
     * Summary: 查询当前数登申请是否支持静默签授权
     * @param QueryautosealauthCopyrightregistrationDigitalregistrationRequest $request
     * @return QueryautosealauthCopyrightregistrationDigitalregistrationResponse
     */
    public function queryautosealauthCopyrightregistrationDigitalregistration($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryautosealauthCopyrightregistrationDigitalregistrationResponse
     */
    public function queryautosealauthCopyrightregistrationDigitalregistrationEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryautosealauthCopyrightregistrationDigitalregistrationResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.copyrightregistration.digitalregistration.queryautosealauth", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 创建数登申请表签署流程
     * Summary: 创建数登申请表签署流程
     * @param CreateapplyformsealflowCopyrightregistrationDigitalregistrationRequest $request
     * @return CreateapplyformsealflowCopyrightregistrationDigitalregistrationResponse
     */
    public function createapplyformsealflowCopyrightregistrationDigitalregistration($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return CreateapplyformsealflowCopyrightregistrationDigitalregistrationResponse
     */
    public function createapplyformsealflowCopyrightregistrationDigitalregistrationEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return CreateapplyformsealflowCopyrightregistrationDigitalregistrationResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.copyrightregistration.digitalregistration.createapplyformsealflow", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 获取签署链接
     * Summary: 获取签署链接
     * @param QueryapplyformsealurlCopyrightregistrationDigitalregistrationRequest $request
     * @return QueryapplyformsealurlCopyrightregistrationDigitalregistrationResponse
     */
    public function queryapplyformsealurlCopyrightregistrationDigitalregistration($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryapplyformsealurlCopyrightregistrationDigitalregistrationResponse
     */
    public function queryapplyformsealurlCopyrightregistrationDigitalregistrationEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryapplyformsealurlCopyrightregistrationDigitalregistrationResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.copyrightregistration.digitalregistration.queryapplyformsealurl", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 完成数登表签署
     * Summary: 完成数登表签署
     * @param CompleteapplyformsealCopyrightregistrationDigitalregistrationRequest $request
     * @return CompleteapplyformsealCopyrightregistrationDigitalregistrationResponse
     */
    public function completeapplyformsealCopyrightregistrationDigitalregistration($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * Summary: 提交DCI凭证核验接口
     * @param VerifyDciCertificateRequest $request
     * @return VerifyDciCertificateResponse
     */
    public function verifyDciCertificate($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return VerifyDciCertificateResponse
     */
    public function verifyDciCertificateEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return VerifyDciCertificateResponse::fromMap($this->doRequest("1.0", "blockchain.bccr.dci.certificate.verify", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 查询DCI凭证核验结果接口
     * Summary: 查询DCI凭证核验结果接口
     * @param QueryDciCertificateresultRequest $request
     * @return QueryDciCertificateresultResponse
     */
    public function queryDciCertificateresult($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
