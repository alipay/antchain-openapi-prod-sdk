...
export class QueryIdentityTagScoreRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
...
export class QueryIdentityTagScoreResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 查询身份标签分数
   */
  async queryIdentityTagScore(request: QueryIdentityTagScoreRequest): Promise<QueryIdentityTagScoreResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryIdentityTagScoreEx(request, headers, runtime);
  }
