...
use AntChain\BOT\Models\ContinuousOtaOpenApiRuleResponse;

class ContinuousOtaOpenApiPolicyResponse extends Model {
    protected $_name = [
        'tenantId' => 'tenant_id',
    ...
    /**
     * @param array $map
     * @return ContinuousOtaOpenApiPolicyResponse
     */
        ...
        if(isset($map['rules'])){
            if(!empty($map['rules'])){
                ...
                foreach($map['rules'] as $item) {
                    $model->rules[$n++] = null !== $item ? ContinuousOtaOpenApiRuleResponse::fromMap($item) : $item;
                }
            }
        }
        return $model;
    ...
    /**
     * @example undefined
     * @var ContinuousOtaOpenApiRuleResponse[]
     */
    public $rules;
