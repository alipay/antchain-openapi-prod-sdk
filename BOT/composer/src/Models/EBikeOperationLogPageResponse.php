...
use AntChain\BOT\Models\EBikeOperationLog;

class EBikeOperationLogPageResponse extends Model {
    protected $_name = [
    ...
    /**
     * @param array $map
     * @return EBikeOperationLogPageResponse
     */
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? EBikeOperationLog::fromMap($item) : $item;
                }
            }
        }
        return $model;
