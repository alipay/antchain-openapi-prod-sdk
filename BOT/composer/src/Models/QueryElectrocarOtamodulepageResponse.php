...
use AntChain\BOT\Models\OTAModuleResponse;

class QueryElectrocarOtamodulepageResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? OTAModuleResponse::fromMap($item) : $item;
                }
            }
        }
