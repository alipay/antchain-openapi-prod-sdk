...
use AntChain\BOT\Models\DeviceOrderResult;
use AntChain\BOT\Models\DeviceOrderFail;

class ImportIotbasicDeviceorderbatchResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['success_list'])){
            if(!empty($map['success_list'])){
                ...
                foreach($map['success_list'] as $item) {
                    $model->successList[$n++] = null !== $item ? DeviceOrderResult::fromMap($item) : $item;
                }
            }
        }
        if(isset($map['fail_list'])){
            if(!empty($map['fail_list'])){
                ...
                foreach($map['fail_list'] as $item) {
                    $model->failList[$n++] = null !== $item ? DeviceOrderFail::fromMap($item) : $item;
                }
            }
        }
        return $model;
