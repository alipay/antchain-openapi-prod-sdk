...
use AntChain\BOT\Models\RegByDeviceIdParm;

class CreateDistributedeviceBydeviceidmulRequest extends Model {
    protected $_name = [
    ...
    /**
     * @param array $map
     * @return CreateDistributedeviceBydeviceidmulRequest
     */
        ...
        if(isset($map['device_param_list'])){
            if(!empty($map['device_param_list'])){
                ...
                foreach($map['device_param_list'] as $item) {
                    $model->deviceParamList[$n++] = null !== $item ? RegByDeviceIdParm::fromMap($item) : $item;
                }
            }
        }
        return $model;
