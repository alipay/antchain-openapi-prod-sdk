...
use AntChain\BOT\Models\XrTicketPoolSuccessList;
use AntChain\BOT\Models\XrTicketPoolFailList;

class BatchcreateXrXrticketpoolResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['success_list'])){
            if(!empty($map['success_list'])){
                ...
                foreach($map['success_list'] as $item) {
                    $model->successList[$n++] = null !== $item ? XrTicketPoolSuccessList::fromMap($item) : $item;
                }
            }
        }
        if(isset($map['fail_list'])){
            if(!empty($map['fail_list'])){
                ...
                foreach($map['fail_list'] as $item) {
                    $model->failList[$n++] = null !== $item ? XrTicketPoolFailList::fromMap($item) : $item;
                }
            }
        }
        return $model;
    ...
    // xr通行证创建成功列表
    /**
     * @var XrTicketPoolSuccessList[]
     */
    public $successList;

    // xr通行证创建失败列表
    /**
     * @var XrTicketPoolFailList[]
     */
    public $failList;
