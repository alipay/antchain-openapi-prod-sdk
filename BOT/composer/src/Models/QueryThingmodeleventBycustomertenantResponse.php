...
use AntChain\BOT\Models\ThingModelEventVO;

class QueryThingmodeleventBycustomertenantResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['event_list'])){
            if(!empty($map['event_list'])){
                ...
                foreach($map['event_list'] as $item) {
                    $model->eventList[$n++] = null !== $item ? ThingModelEventVO::fromMap($item) : $item;
                }
            }
        }
        return $model;
