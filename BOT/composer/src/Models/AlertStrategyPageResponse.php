...
use AntChain\BOT\Models\AlertStrategy;

class AlertStrategyPageResponse extends Model {
    protected $_name = [
    ...
    /**
     * @param array $map
     * @return AlertStrategyPageResponse
     */
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? AlertStrategy::fromMap($item) : $item;
                }
            }
        }
        return $model;
