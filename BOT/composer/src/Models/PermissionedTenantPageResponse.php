...
use AntChain\BOT\Models\PermissionedTenantModel;

class PermissionedTenantPageResponse extends Model {
    protected $_name = [
    ...
    /**
     * @param array $map
     * @return PermissionedTenantPageResponse
     */
        ...
        if(isset($map['page_data'])){
            if(!empty($map['page_data'])){
                ...
                foreach($map['page_data'] as $item) {
                    $model->pageData[$n++] = null !== $item ? PermissionedTenantModel::fromMap($item) : $item;
                }
            }
        }
        return $model;
