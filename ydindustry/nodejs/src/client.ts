...
export class QueryRetailScoreResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 蚁盾零售行业评分
   */
  async queryRetailScore(request: QueryRetailScoreRequest): Promise<QueryRetailScoreResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryRetailScoreEx(request, headers, runtime);
  }
