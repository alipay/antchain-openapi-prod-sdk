...
class ListSgAuditlogsResponse extends Model
{
    // 请求唯一ID，用于链路跟踪和问题排查
    /**
     * @var string
        ...
        if (null !== $this->auditLogs) {
            ...
            if (null !== $this->auditLogs && \is_array($this->auditLogs)) {
                ...
                foreach ($this->auditLogs as $item) {
                    $res['audit_logs'][$n++] = null !== $item ? $item->toMap() : $item;
                }
            }
        }
    ...
    public static function fromMap($map = [])
    {
        ...
        if (isset($map['audit_logs'])) {
            if (!empty($map['audit_logs'])) {
                $model->auditLogs = [];
                $n                = 0;
                foreach ($map['audit_logs'] as $item) {
                    $model->auditLogs[$n++] = null !== $item ? AuditLogModel::fromMap($item) : $item;
                }
            }
        }
