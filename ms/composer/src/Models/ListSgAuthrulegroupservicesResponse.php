...
class ListSgAuthrulegroupservicesResponse extends Model
{
    // 请求唯一ID，用于链路跟踪和问题排查
    /**
     * @var string
        ...
        if (isset($map['auth_rule_groups'])) {
            if (!empty($map['auth_rule_groups'])) {
                ...
                foreach ($map['auth_rule_groups'] as $item) {
                    $model->authRuleGroups[$n++] = null !== $item ? AuthRuleGroupListModelVO::fromMap($item) : $item;
                }
            }
        }
