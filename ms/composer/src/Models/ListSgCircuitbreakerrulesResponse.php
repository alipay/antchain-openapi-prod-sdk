...
class ListSgCircuitbreakerrulesResponse extends Model
{
    // 请求唯一ID，用于链路跟踪和问题排查
    /**
     * @var string
        ...
        if (isset($map['rule_models'])) {
            if (!empty($map['rule_models'])) {
                ...
                foreach ($map['rule_models'] as $item) {
                    $model->ruleModels[$n++] = null !== $item ? CircuitBreakerRuleResult::fromMap($item) : $item;
                }
            }
        }
