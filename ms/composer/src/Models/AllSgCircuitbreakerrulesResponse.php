...
class AllSgCircuitbreakerrulesResponse extends Model
{
    // 请求唯一ID，用于链路跟踪和问题排查
    /**
     * @var string
    ...
    // 应用名及规则数
    /**
     * @var CircuitBreakerRuleAppNamesModel[]
     */
    public $ruleModels;
        ...
        if (isset($map['rule_models'])) {
            if (!empty($map['rule_models'])) {
                ...
                foreach ($map['rule_models'] as $item) {
                    $model->ruleModels[$n++] = null !== $item ? CircuitBreakerRuleAppNamesModel::fromMap($item) : $item;
                }
            }
        }
