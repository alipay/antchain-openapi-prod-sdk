...
export class QueryDemoGatewayTestRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  productInstanceId?: string;
  static names(): { [key: string]: string } {
...
export class QueryDemoGatewayTestResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 测试001
   */
  async queryDemoGatewayTest(request: QueryDemoGatewayTestRequest): Promise<QueryDemoGatewayTestResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryDemoGatewayTestEx(request, headers, runtime);
  }
   ...
   * Summary: 测试001
   */
  async queryDemoGatewayTestEx(request: QueryDemoGatewayTestRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<QueryDemoGatewayTestResponse> {
    Util.validateModel(request);
    return $tea.cast<QueryDemoGatewayTestResponse>(await this.doRequest("1.0", "demo.gateway.test.query", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new QueryDemoGatewayTestResponse({}));
  }
