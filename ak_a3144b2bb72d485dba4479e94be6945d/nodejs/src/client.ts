...
export class QueryDemoAaaAaaRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  productInstanceId?: string;
  static names(): { [key: string]: string } {
...
export class QueryDemoAaaAaaResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: test
   */
  async queryDemoAaaAaa(request: QueryDemoAaaAaaRequest): Promise<QueryDemoAaaAaaResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryDemoAaaAaaEx(request, headers, runtime);
  }
   ...
   * Summary: test
   */
  async queryDemoAaaAaaEx(request: QueryDemoAaaAaaRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<QueryDemoAaaAaaResponse> {
    Util.validateModel(request);
    return $tea.cast<QueryDemoAaaAaaResponse>(await this.doRequest("1.0", "demo.aaa.aaa.query", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new QueryDemoAaaAaaResponse({}));
  }
