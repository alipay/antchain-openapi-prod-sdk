...
use AntChain\STLR\Models\QueryProductDataRequest;
use AntChain\STLR\Models\QueryProductDataResponse;
use AntChain\STLR\Models\PushProductDataRequest;
use AntChain\STLR\Models\PushProductDataResponse;
use AntChain\STLR\Models\QueryActiveDataRequest;
use AntChain\STLR\Models\QueryActiveDataResponse;
use AntChain\STLR\Models\ExecActiveDataRequest;
...
class Client {
    ...
    public function uploadEcarFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadEcarFileResponse = new UploadEcarFileResponse([
    ...
    public function uploadEcarPlaformfileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadEcarPlaformfileResponse = new UploadEcarPlaformfileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function submitEcarGreencertificategenerationfileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $submitEcarGreencertificategenerationfileResponse = new SubmitEcarGreencertificategenerationfileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
     ...
     * Summary: 产品产量填报检查接口
     * @param QueryProductDataRequest $request
     * @return QueryProductDataResponse
     */
    public function queryProductData($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->queryProductDataEx($request, $headers, $runtime);
    }
     ...
     * @return QueryProductDataResponse
     */
    public function queryProductDataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryProductDataResponse::fromMap($this->doRequest("1.0", "antchain.carbon.product.data.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 协鑫数据直连产品产量填报接口
     * Summary: 协鑫数据直连产品产量填报接口
     * @param PushProductDataRequest $request
     * @return PushProductDataResponse
     */
    public function pushProductData($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->pushProductDataEx($request, $headers, $runtime);
    }
     ...
     * @return PushProductDataResponse
     */
    public function pushProductDataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return PushProductDataResponse::fromMap($this->doRequest("1.0", "antchain.carbon.product.data.push", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 协鑫数据直连lca活动数据项查询
     * Summary: 协鑫数据直连lca活动数据项查询
     * @param QueryActiveDataRequest $request
     * @return QueryActiveDataResponse
     */
    public function queryActiveData($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->queryActiveDataEx($request, $headers, $runtime);
    }
     ...
     * @return QueryActiveDataResponse
     */
    public function queryActiveDataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryActiveDataResponse::fromMap($this->doRequest("1.0", "antchain.carbon.active.data.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 协鑫数据直连lca活动数据检查接口
     * Summary: 协鑫数据直连lca活动数据检查接口
     * @param ExecActiveDataRequest $request
     * @return ExecActiveDataResponse
     */
    public function execActiveData($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->execActiveDataEx($request, $headers, $runtime);
    }
     ...
     * @return ExecActiveDataResponse
     */
    public function execActiveDataEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return ExecActiveDataResponse::fromMap($this->doRequest("1.0", "antchain.carbon.active.data.exec", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 协鑫数据直连lca活动数据填报
     * Summary: 协鑫数据直连lca活动数据填报
     * @param PushActiveDataRequest $request
     * @return PushActiveDataResponse
     */
    public function pushActiveData($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
        return $this->pushActiveDataEx($request, $headers, $runtime);
    }
