...
export class CallbackRiskplusMdipFileRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
...
export class CallbackRiskplusMdipFileResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
   ...
   * Summary: 支持异步回传文件
   */
  async callbackRiskplusMdipFile(request: CallbackRiskplusMdipFileRequest): Promise<CallbackRiskplusMdipFileResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.callbackRiskplusMdipFileEx(request, headers, runtime);
  }
   ...
   * Summary: 支持异步回传文件
   */
  async callbackRiskplusMdipFileEx(request: CallbackRiskplusMdipFileRequest, headers: {[key: string ]: string}, runtime: $Util.RuntimeOptions): Promise<CallbackRiskplusMdipFileResponse> {
    if (!Util.isUnset(request.fileObject)) {
      ...
      if (!AntchainUtil.isSuccess(uploadResp.resultCode, "ok")) {
        let callbackRiskplusMdipFileResponse = new CallbackRiskplusMdipFileResponse({
    ...
    return $tea.cast<CallbackRiskplusMdipFileResponse>(await this.doRequest("1.0", "riskplus.mdip.file.callback", "HTTPS", "POST", `/gateway.do`, $tea.toMap(request), headers, runtime), new CallbackRiskplusMdipFileResponse({}));
  }
