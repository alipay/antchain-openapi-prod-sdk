...
use AntChain\RISKPLUS\Models\CustomerRpspInfosModel;

class BatchqueryQmpRtMixedmarketingResponse extends Model {
    protected $_name = [
        'reqMsgId' => 'req_msg_id',
        ...
        if(isset($map['query_results'])){
            if(!empty($map['query_results'])){
                ...
                foreach($map['query_results'] as $item) {
                    $model->queryResults[$n++] = null !== $item ? CustomerRpspInfosModel::fromMap($item) : $item;
                }
            }
        }
        return $model;
