...
use AntChain\RISKPLUS\Models\ExecFlowRunRequest;
use AntChain\RISKPLUS\Models\ExecFlowRunResponse;
use AntChain\RISKPLUS\Models\QueryFlowRunRequest;
use AntChain\RISKPLUS\Models\QueryFlowRunResponse;
...
use AntChain\RISKPLUS\Models\SyncDubbridgeEcmonitorRequest;
use AntChain\RISKPLUS\Models\SyncDubbridgeEcmonitorResponse;
...
use AntChain\RISKPLUS\Models\ApplyDubbridgePetcashierRequest;
...
use AntChain\RISKPLUS\Models\CallbackMdipFileRequest;
use AntChain\RISKPLUS\Models\CallbackMdipFileResponse;
...
use AntChain\RISKPLUS\Models\QueryCreditshieldRestrictedRequest;
use AntChain\RISKPLUS\Models\QueryCreditshieldRestrictedResponse;
...
use AntChain\RISKPLUS\Models\QueryCreditshieldFixedcontactResponse;
...
use AntChain\RISKPLUS\Models\PushCreditshieldAllocatecaseRequest;
use AntChain\RISKPLUS\Models\PushCreditshieldAllocatecaseResponse;
use AntChain\RISKPLUS\Models\PushCreditshieldReturncaseRequest;
use AntChain\RISKPLUS\Models\PushCreditshieldReturncaseResponse;
use AntChain\RISKPLUS\Models\QueryCreditshieldFundcallbackRequest;
use AntChain\RISKPLUS\Models\QueryCreditshieldFundcallbackResponse;
...
use AntChain\RISKPLUS\Models\PushTdiaiworkshopcloudTestRequest;
use AntChain\RISKPLUS\Models\PushTdiaiworkshopcloudTestResponse;
use AntChain\RISKPLUS\Models\QueryTdiaiworkshopcloudBatchRequest;
use AntChain\RISKPLUS\Models\QueryTdiaiworkshopcloudBatchResponse;
use AntChain\RISKPLUS\Models\PushTdiaiworkshopcloudBatchRequest;
use AntChain\RISKPLUS\Models\PushTdiaiworkshopcloudBatchResponse;
...
class Client {
     ...
     * @param ExecFlowRunRequest $request
     * @return ExecFlowRunResponse
     */
    public function execFlowRun($request){
     ...
     * @return ExecFlowRunResponse
     */
    public function execFlowRunEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return ExecFlowRunResponse::fromMap($this->doRequest("1.0", "riskplus.flow.run.exec", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 查询任务执行状态或者结果
     * Summary: 查询任务执行状态或者结果
     * @param QueryFlowRunRequest $request
     * @return QueryFlowRunResponse
     */
    public function queryFlowRun($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryFlowRunResponse
     */
    public function queryFlowRunEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryFlowRunResponse::fromMap($this->doRequest("1.0", "riskplus.flow.run.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
    ...
    public function uploadDubbridgeFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadDubbridgeFileResponse = new UploadDubbridgeFileResponse([
     ...
     * @param SyncDubbridgeEcmonitorRequest $request
     * @return SyncDubbridgeEcmonitorResponse
     */
    public function syncDubbridgeEcmonitor($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return SyncDubbridgeEcmonitorResponse
     */
    public function syncDubbridgeEcmonitorEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return SyncDubbridgeEcmonitorResponse::fromMap($this->doRequest("1.0", "riskplus.dubbridge.ecmonitor.sync", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
     ...
     * Description: 聚合收银台申请
     * Summary: 聚合收银台申请
     * @param ApplyDubbridgePetcashierRequest $request
     * @return ApplyDubbridgePetcashierResponse
     */
    public function applyDubbridgePetcashier($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return ApplyDubbridgePetcashierResponse
     */
    public function applyDubbridgePetcashierEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return ApplyDubbridgePetcashierResponse::fromMap($this->doRequest("1.0", "riskplus.dubbridge.petcashier.apply", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 交易订单查询
     * Summary: 交易订单查询
     * @param QueryDubbridgePetorderRequest $request
     * @return QueryDubbridgePetorderResponse
     */
    public function queryDubbridgePetorder($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
    ...
    public function receiveMdipParamsFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $receiveMdipParamsFileResponse = new ReceiveMdipParamsFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function receiveMdipParamsRbbfileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $receiveMdipParamsRbbfileResponse = new ReceiveMdipParamsRbbfileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
     ...
     * Summary: 支持异步回传文件
     * @param CallbackMdipFileRequest $request
     * @return CallbackMdipFileResponse
     */
    public function callbackMdipFile($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return CallbackMdipFileResponse
     */
    public function callbackMdipFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            $uploadReq = new CreateAntcloudGatewayxFileUploadRequest([
                "authToken" => $request->authToken,
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $callbackMdipFileResponse = new CallbackMdipFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
        ...
        Utils::validateModel($request);
        return CallbackMdipFileResponse::fromMap($this->doRequest("1.0", "riskplus.mdip.file.callback", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
    ...
    public function uploadQmpOfflinehostplanEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadQmpOfflinehostplanResponse = new UploadQmpOfflinehostplanResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function receiveRfcParamsFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $receiveRfcParamsFileResponse = new ReceiveRfcParamsFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function uploadRfcAiboundFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadRfcAiboundFileResponse = new UploadRfcAiboundFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function uploadRfcAiboundConvertEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadRfcAiboundConvertResponse = new UploadRfcAiboundConvertResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function uploadRbbFileAmapEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadRbbFileAmapResponse = new UploadRbbFileAmapResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function receiveRbbParamsFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $receiveRbbParamsFileResponse = new ReceiveRbbParamsFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function receiveRbbOverseacompanyProfileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $receiveRbbOverseacompanyProfileResponse = new ReceiveRbbOverseacompanyProfileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
     ...
     * @param QueryCreditshieldRestrictedRequest $request
     * @return QueryCreditshieldRestrictedResponse
     */
    public function queryCreditshieldRestricted($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryCreditshieldRestrictedResponse
     */
    public function queryCreditshieldRestrictedEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryCreditshieldRestrictedResponse::fromMap($this->doRequest("1.0", "riskplus.creditshield.restricted.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 可信联系方式查询独立服务接口
     * Summary: 可信联系方式查询独立服务接口
     * @param QueryCreditshieldFixcontactRequest $request
     * @return QueryCreditshieldFixcontactResponse
     */
    public function queryCreditshieldFixcontact($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryCreditshieldFixcontactResponse
     */
    public function queryCreditshieldFixcontactEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryCreditshieldFixcontactResponse::fromMap($this->doRequest("1.0", "riskplus.creditshield.fixcontact.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 可信联系方式-已修复手机号批量查询
     * Summary: 可信联系方式-已修复手机号批量查询
     * @param QueryCreditshieldFixedcontactRequest $request
     * @return QueryCreditshieldFixedcontactResponse
     */
    public function queryCreditshieldFixedcontact($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryCreditshieldFixedcontactResponse
     */
    public function queryCreditshieldFixedcontactEx($request, $headers, $runtime){
        Utils::validateModel($request);
     ...
     * Description: 接受案件信息
     * Summary: 接受案件信息
     * @param PushCreditshieldAllocatecaseRequest $request
     * @return PushCreditshieldAllocatecaseResponse
     */
    public function pushCreditshieldAllocatecase($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return PushCreditshieldAllocatecaseResponse
     */
    public function pushCreditshieldAllocatecaseEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return PushCreditshieldAllocatecaseResponse::fromMap($this->doRequest("1.0", "riskplus.creditshield.allocatecase.push", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 退案接口
     * Summary: 退案接口
     * @param PushCreditshieldReturncaseRequest $request
     * @return PushCreditshieldReturncaseResponse
     */
    public function pushCreditshieldReturncase($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return PushCreditshieldReturncaseResponse
     */
    public function pushCreditshieldReturncaseEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return PushCreditshieldReturncaseResponse::fromMap($this->doRequest("1.0", "riskplus.creditshield.returncase.push", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 资方回调
     * Summary: 资方回调
     * @param QueryCreditshieldFundcallbackRequest $request
     * @return QueryCreditshieldFundcallbackResponse
     */
    public function queryCreditshieldFundcallback($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryCreditshieldFundcallbackResponse
     */
    public function queryCreditshieldFundcallbackEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryCreditshieldFundcallbackResponse::fromMap($this->doRequest("1.0", "riskplus.creditshield.fundcallback.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
     ...
     * Description: 接口创建测试
     * Summary: 接口创建测试
     * @param PushTdiaiworkshopcloudTestRequest $request
     * @return PushTdiaiworkshopcloudTestResponse
     */
    public function pushTdiaiworkshopcloudTest($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return PushTdiaiworkshopcloudTestResponse
     */
    public function pushTdiaiworkshopcloudTestEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return PushTdiaiworkshopcloudTestResponse::fromMap($this->doRequest("1.0", "riskplus.tdiaiworkshopcloud.test.push", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 风控离线批量样本数据查询接口
     * Summary: 风控离线批量样本数据查询接口
     * @param QueryTdiaiworkshopcloudBatchRequest $request
     * @return QueryTdiaiworkshopcloudBatchResponse
     */
    public function queryTdiaiworkshopcloudBatch($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryTdiaiworkshopcloudBatchResponse
     */
    public function queryTdiaiworkshopcloudBatchEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryTdiaiworkshopcloudBatchResponse::fromMap($this->doRequest("1.0", "riskplus.tdiaiworkshopcloud.batch.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 风控离线批量样本数据上传接口
     * Summary: 风控离线批量样本数据上传接口
     * @param PushTdiaiworkshopcloudBatchRequest $request
     * @return PushTdiaiworkshopcloudBatchResponse
     */
    public function pushTdiaiworkshopcloudBatch($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return PushTdiaiworkshopcloudBatchResponse
     */
    public function pushTdiaiworkshopcloudBatchEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return PushTdiaiworkshopcloudBatchResponse::fromMap($this->doRequest("1.0", "riskplus.tdiaiworkshopcloud.batch.push", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
    ...
    public function uploadUmktParamsFileEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadUmktParamsFileResponse = new UploadUmktParamsFileResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function uploadUmktOfflinedecisionEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadUmktOfflinedecisionResponse = new UploadUmktOfflinedecisionResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function uploadUmktOfflineImportrecordEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $uploadUmktOfflineImportrecordResponse = new UploadUmktOfflineImportrecordResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
