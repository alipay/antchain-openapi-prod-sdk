...
class ListUniversalsaasDigitalhumanLlmModelResponse extends Model
{
    // 请求唯一ID，用于链路跟踪和问题排查
    /**
     * @var string
        ...
        if (isset($map['item_list'])) {
            if (!empty($map['item_list'])) {
                ...
                foreach ($map['item_list'] as $item) {
                    $model->itemList[$n++] = null !== $item ? AvatarLlmModelInfo::fromMap($item) : $item;
                }
            }
        }
