...
export class ServiceAntcloudOpenapiGatewayCheckRequest extends $tea.Model {
  // OAuth模式下的授权token
  authToken?: string;
  static names(): { [key: string]: string } {
    return {
...
export class ServiceAntcloudOpenapiGatewayCheckResponse extends $tea.Model {
  // 请求唯一ID，用于链路跟踪和问题排查
  reqMsgId?: string;
  // 结果码，一般OK表示调用成功
  resultCode?: string;
...
export default class Client {
  ...
  async serviceAntcloudOpenapiGatewayCheck(request: ServiceAntcloudOpenapiGatewayCheckRequest): Promise<ServiceAntcloudOpenapiGatewayCheckResponse> {
    ...
    return await this.serviceAntcloudOpenapiGatewayCheckEx(request, headers, runtime);
  }
   ...
   * Summary: ss
   */
  async queryDemoGatewayCheck(request: QueryDemoGatewayCheckRequest): Promise<QueryDemoGatewayCheckResponse> {
    let runtime = new $Util.RuntimeOptions({ });
    let headers : {[key: string ]: string} = { };
    return await this.queryDemoGatewayCheckEx(request, headers, runtime);
  }
