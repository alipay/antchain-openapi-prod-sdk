...
use AntChain\REALPERSON\Models\QueryThreemetaPhonereuseproxRequest;
use AntChain\REALPERSON\Models\QueryThreemetaPhonereuseproxResponse;
use AntChain\REALPERSON\Models\QueryThreemetaPhonereusexRequest;
use AntChain\REALPERSON\Models\QueryThreemetaPhonereusexResponse;
use AntChain\REALPERSON\Models\InitServerWillauthRequest;
use AntChain\REALPERSON\Models\InitServerWillauthResponse;
use AntChain\REALPERSON\Models\QueryServerWillauthRequest;
use AntChain\REALPERSON\Models\QueryServerWillauthResponse;
use AntChain\REALPERSON\Models\QueryServerWillauthmaterialRequest;
use AntChain\REALPERSON\Models\QueryServerWillauthmaterialResponse;
...
class Client {
    ...
    public function createVoiceprintServermodeEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $createVoiceprintServermodeResponse = new CreateVoiceprintServermodeResponse([
    ...
    public function recognizeDocIndividualcardEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $recognizeDocIndividualcardResponse = new RecognizeDocIndividualcardResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function execFacevrfServermodeEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $execFacevrfServermodeResponse = new ExecFacevrfServermodeResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function createFaceverifyServerEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $createFaceverifyServerResponse = new CreateFaceverifyServerResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function execFaceverifyServermodeEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $execFaceverifyServermodeResponse = new ExecFaceverifyServermodeResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function scaleinImageEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $scaleinImageResponse = new ScaleinImageResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function recognizeOcrIndividualcardEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $recognizeOcrIndividualcardResponse = new RecognizeOcrIndividualcardResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
    ...
    public function queryRiskServerEx($request, $headers, $runtime){
        if (!Utils::isUnset($request->fileObject)) {
            ...
            $uploadResp = $this->createAntcloudGatewayxFileUploadEx($uploadReq, $headers, $runtime);
            if (!UtilClient::isSuccess($uploadResp->resultCode, "ok")) {
                $queryRiskServerResponse = new QueryRiskServerResponse([
                    "reqMsgId" => $uploadResp->reqMsgId,
                    "resultCode" => $uploadResp->resultCode,
                    "resultMsg" => $uploadResp->resultMsg
                ]);
     ...
     * @param QueryThreemetaPhonereuseproxRequest $request
     * @return QueryThreemetaPhonereuseproxResponse
     */
    public function queryThreemetaPhonereuseprox($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryThreemetaPhonereuseproxResponse
     */
    public function queryThreemetaPhonereuseproxEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryThreemetaPhonereuseproxResponse::fromMap($this->doRequest("1.0", "di.realperson.threemeta.phonereuseprox.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 运营商二次放号扩展接口
     * Summary: 运营商二次放号扩展接口
     * @param QueryThreemetaPhonereusexRequest $request
     * @return QueryThreemetaPhonereusexResponse
     */
    public function queryThreemetaPhonereusex($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryThreemetaPhonereusexResponse
     */
    public function queryThreemetaPhonereusexEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryThreemetaPhonereusexResponse::fromMap($this->doRequest("1.0", "di.realperson.threemeta.phonereusex.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
     ...
     * Summary: 意愿认证服务端查询
     * @param QueryServerWillauthRequest $request
     * @return QueryServerWillauthResponse
     */
    public function queryServerWillauth($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryServerWillauthResponse
     */
    public function queryServerWillauthEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryServerWillauthResponse::fromMap($this->doRequest("1.0", "di.realperson.server.willauth.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }

    /**
     * Description: 意愿认证服务端材料查询
     * Summary: 意愿认证服务端材料查询
     * @param QueryServerWillauthmaterialRequest $request
     * @return QueryServerWillauthmaterialResponse
     */
    public function queryServerWillauthmaterial($request){
        $runtime = new RuntimeOptions([]);
        $headers = [];
     ...
     * @return QueryServerWillauthmaterialResponse
     */
    public function queryServerWillauthmaterialEx($request, $headers, $runtime){
        Utils::validateModel($request);
        return QueryServerWillauthmaterialResponse::fromMap($this->doRequest("1.0", "di.realperson.server.willauthmaterial.query", "HTTPS", "POST", "/gateway.do", Tea::merge($request), $headers, $runtime));
    }
